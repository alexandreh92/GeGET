...
namespace GeGET
{
    public partial class Itens : UserControl, IDisposable
    {
        #region Declarations
        bool disposed = false;
        ...
        private void Commit()
        {
            Dispatcher.Invoke(DispatcherPriority.Background,
                  new Action(() =>
                  {
                      ...
                      var filtered = listaItens.Where(descricao => Find.All(list => descricao.Descricao.ToLower().RemoveDiacritics().Contains(list) || descricao.Grupo_Descricao.ToLower().RemoveDiacritics().Contains(list) || descricao.Usuario.RemoveDiacritics().ToLower().Contains(list)));

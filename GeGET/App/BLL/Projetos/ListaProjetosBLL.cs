...
namespace BLL
{
    class ListaProjetosBLL
    {
        ...
        public ObservableCollection<ListaProjetosDTO> LoadLista(InformacoesListaProjetosDTO DTO)
        {
            ...
            try
            {
                var query = "SELECT (@cnt := @cnt + 1) AS num, t.* FROM (SELECT p.id, i.descricao, p.partnumber, un.descricao as un, f.rsocial,lv.quantidade_orc, lv.preco_orc, lv.anotacoes, lv.quantidade, lv.quantidade * lv.preco as preco_total, lv.preco, lv.fd, lv.id as pk FROM lista_vendas lv JOIN produto p ON lv.PRODUTO_id = p.id JOIN item i ON p.DESCRICAO_ITEM_id = i.id JOIN fornecedor f ON p.FORNECEDOR_id = f.id JOIN unidade un ON un.id = i.unidade_id JOIN vendas vnd ON vnd.id = lv.vendas_id WHERE vnd.NEGOCIO_id = '" + DTO.Negocio_Id + "' AND lv.ATIVIDADE_id = '" + DTO.Atividade_Id + "') t CROSS JOIN(SELECT @cnt:= 0) AS dummy";
                bd.Conectar();
    ...
    class InformacoesListaProjetosBLL
    {
        AcessoBancoDados bd = new AcessoBancoDados();

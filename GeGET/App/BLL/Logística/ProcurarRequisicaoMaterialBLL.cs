...
namespace BLL
{
    class ProcurarRequisicaoMaterialBLL : IDisposable
    {
        ...
        public ObservableCollection<ProcurarRequisicaoMaterialDTO> LoadRMConsulta()
        {
            ...
            try
            {
                var query = "SELECT rm.id,n.id AS negocio_id,c.rsocial,rm.description,n.versao_valida,cid.cidade,cid.uf,f.nome,e.endereco,vendas_id FROM requisicao_material rm JOIN vendas v ON v.id=rm.vendas_id JOIN negocio n ON n.id=v.negocio_id JOIN estabelecimento e ON n.estabelecimento_id=e.id JOIN cliente c ON c.id=e.cliente_id JOIN usuario u ON u.id=rm.usuario_id JOIN funcionario f ON f.id=u.funcionario_id JOIN cidades cid ON cid.id=e.cidades_id JOIN estados est ON est.id=e.estados_id ORDER BY id ASC";
                bd.Conectar();
                dt = bd.RetDataTable(query);
            }
            catch (Exception ex)
